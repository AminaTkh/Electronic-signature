...
namespace Block
{
    ...
    class Block
    {
      ...
      public :
        ...
        uint8_t get_byte (size_t) const;
        void set_byte (uint8_t, size_t);
        const uint8_t* get_bitset () const;
...
template <size_t S>
Block<S>::operator uint8_t () const
{
    return *((uint8_t*)bitset);
}

template <size_t S>
Block<S>::operator uint64_t () const
{
...
template <size_t S>
Block<S> Block<S>::operator ^ (const Block<S>& block) const
{
    Block<S> new_block;
    for (size_t i = 0; i < S / BYTE_SIZE; ++i) {
...
template <size_t S>
Block<S> Block<S>::operator * (bool symb) const
{
    if (!symb) {
        return Block<S>(0);
    }
...
template <size_t S>
bool Block<S>::operator [] (size_t i) const 
{
...
template <size_t S>
bool Block<S>::operator == (const Block<S> &block) const
{
    for (size_t i = 0; i < S / BYTE_SIZE; ++i) {
...
template <size_t S>
uint8_t Block<S>::get_byte (size_t i) const
{
    return bitset[i];
}

template <size_t S>
void Block<S>::set_byte (uint8_t val, size_t i)
{
    bitset[i] = val;
}

template <size_t S>
const uint8_t* Block<S>::get_bitset () const
{
    return bitset;
}
