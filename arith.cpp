...
static uint1024_t multiplication (const uint1024_t &f1, uint8_t f2)
{
    uint8_t flag = 0;
    uint8_t result[2 * UINT512_SIZE] = {0};
    for (size_t i = 0; i < 2 * UINT512_SIZE; ++i) {
        result[i] = (uint8_t)((f1.get_byte(i) * f2 + flag) % MOD);
        flag = (uint8_t)((f1.get_byte(i) * f2 + flag) / MOD);
    }
...
template <size_t S>
static Block ::Block <S> addition (const Block ::Block <S> &a1, const Block ::Block <S> &a2)
{
    uint32_t flag = 0;
    uint8_t result[S / BYTE_SIZE] = {0};
    for (size_t i = 0; i < S / BYTE_SIZE; ++i) {
        result[i] = (uint8_t)(((uint32_t)a1.get_byte(i) + (uint32_t)a2.get_byte(i) + flag) % MOD);
        flag = ((uint32_t)a1.get_byte(i) + (uint32_t)a2.get_byte(i) + flag) / MOD;
    }
    return Block ::Block <S>(result, S / BYTE_SIZE);
}

uint1024_t multiplication (const uint512_t &f1, const uint512_t &f2)
{
    uint1024_t result = 0;
    size_t j = 0;
    size_t m = set_size(f1);
    size_t n = set_size(f2);
    while (j < n) {
        if (f2.get_byte(j) == 0) {
            ...
            uint32_t k = 0;
            while (i < m) {
                uint32_t t = f1.get_byte(i) * f2.get_byte(j) + result.get_byte(i + j) + k;
...
uint512_t inverse (const uint512_t &num, const uint512_t &modik)
{
    uint1024_t r0 = extend(modik);
    ...
    if (cnt % 2) {
        ...
        auto result = substraction(modik, p[(3 + i - 1) % 3]);
        return result.split<512>()[0];
    }   
}

uint512_t opposite (const uint512_t &num, const uint512_t &modik)
{
    return substraction(modik, num);
}

uint512_t multiplication (const uint512_t &f1, const uint512_t &f2, const uint512_t &modik)
{
    return (multiplication(f1, f2) % extend(modik)).split<512>()[0];
}

uint512_t addition (const uint512_t &a1, const uint512_t &a2, const uint512_t &modik)
{
    return (addition(extend(a1), extend(a2)) % extend(modik)).split<512>()[0];
}

uint512_t substraction (const uint512_t &m, const uint512_t &s, const uint512_t &modik)
{
    return addition(m, opposite(s, modik), modik);
}
